	...
	public synchronized void handleTextMessage(WebSocketSession session, TextMessage wsMsg) throws Exception {
		...
		try {
			...
			case MARK:
				...
				if (game.checkDraw()) {
					statistics.addTied(player1);
					statistics.addTied(player2);
				}else{
					if(game.checkWinner().win){
						if(game.checkTurn(player1.getId())) {
							statistics.addWin(player1);
							statistics.addLoss(player2);
						}else {
							statistics.addWin(player2);
							statistics.addLoss(player1);
						}
					}
				}
				break;
